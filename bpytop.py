...
args.add_argument("-b", "--boxes", action="store", dest="boxes",
...
class Config:
	...
	keys: List[str] = ["color_theme", "update_ms", "proc_sorting", "proc_reversed", "proc_tree", "check_temp",
					   "draw_clock", "background_update", "custom_cpu_name",
					   "proc_colors", "proc_gradient", "proc_per_core", "proc_mem_bytes", "disks_filter",
					   "update_check", "log_level", "mem_graphs", "show_swap",
					   "swap_disk", "show_disks", "use_fstab", "net_download", "net_upload", "net_auto",
	...
	sorting_options: List[str] = ["pid", "program", "arguments", "threads", "user", "memory", "cpu lazy",
								  "cpu responsive"]
			...
			for box_class in Box.__subclasses__():
				for box_name in Box.boxes:
					if box_name in str(box_class).capitalize():
						if not (box_name == "cpu" and "proc" in Box.boxes) and not (
							box_name == "net" and "mem" in Box.boxes) and w_p + box_class.width_p <= 100:
			...
			while not cls.stopping:
				with Raw(sys.stdin):
					if not select([sys.stdin], [], [], 0.1)[
						0]:  # * Wait 100ms for input on stdin then restart loop to check for stop flag
						continue
					...
					if input_key == "\033":
						...
						("\033[<0;", "\033[<35;", "\033[<64;", "\033[<65;")):  # * Detected mouse event
						try:
							...
							if input_key.startswith("\033[<35;"):  # * Detected mouse move in mouse direct mode
								...
								"m"):  # * Detected mouse click release
								if Menu.active:
...
class Draw:
	...
	* .last_screen() : Prints all saved buffers
	...
	def buffer(cls, name: str, *args: str, append: bool = False, now: bool = False, z: int = 100,
			   only_save: bool = False, no_save: bool = False, once: bool = False):
		string: str = ""
		if name.startswith("+"):
			name = name.lstrip("+")
			append = True
		...
		try:
			if not color:
				...
				if len(self.hexa) == 3:
					...
					self.dec = (
						int(self.hexa[1:3], base=16), int(self.hexa[3:5], base=16), int(self.hexa[5:7], base=16))
				else:
...
class Theme:
	...
	def _load_theme(self, theme: str):
		...
		for name in self.gradient:
			rgb = {"start": getattr(self, f'{name}_start').dec, "mid": getattr(self, f'{name}_mid').dec,
...
class Graph:
	...
	* __call__ : same as add
	...
	def __init__(self, width: int, height: int, color: Union[List[str], Color, None], data: List[int],
				 invert: bool = False, max_value: int = 0, offset: int = 0, color_max_value: Union[int, None] = None,
				 no_zero: bool = False, round_up_low: bool = False):
		...
		if max_value:
			...
			data = [min_max((v + offset) * 100 // (max_value + offset), min_max(v + offset, 0, self.lowest), 100) for v
					in data]  # * Convert values to percentage values of max_value with max_value as ceiling
		else:
			self.max_value = 0
		if color_max_value:
		...
		if isinstance(color, list) and height > 1:
			for i in range(1, height + 1): self.colors.insert(0, color[
				min(100, i * color_scale // height)])  # * Calculate colors of graph
				...
				for val, side in [self.last, "left"], [data[v], "right"]:  # type: ignore
					if val >= h_high:
						...
						if self.height == 1:
							...
							value[side] = round((val - h_low) * 4 / (h_high - h_low) + 0.1)
					if self.no_zero and not (new and v == 0 and side == "left") and h == self.height - 1 and value[
						side] < 1 and not (self.round_up_low and val == 0): value[side] = 1
	...
	def __call__(self, value: Union[int, None] = None) -> str:
		...
		if self.max_value: value = min_max((value + self.offset) * 100 // (self.max_value + self.offset),
										   min_max(value + self.offset, 0, self.lowest), 100)
		self._create([value])
		return self.out
...
class Box:
	...
	num: int = 0
	boxes: List = []
	view_modes: Dict[str, List] = {"full": ["cpu", "mem", "net", "proc"], "stat": ["cpu", "mem", "net"],
								   "proc": ["cpu", "proc"]}
	view_mode: str
	...
	def draw_update_ms(cls, now: bool = True):
		...
		Draw.buffer("update_ms!" if now and not Menu.active else "update_ms",
					...
					only_save=Menu.active, once=True)
...
class CpuBox(Box, SubBox):
	...
	def _calc_size(cls):
		...
		if cls.box_columns * (20 + 13 if cpu.got_sensors else 21) < cls.width - (cls.width // 3):
			...
			cls.box_columns = (cls.width - cls.width // 3) // (8 + 6 if cpu.got_sensors else 8);
			cls.column_size = 0
	...
	def _draw_fg(cls):
		...
		if cls.resized or cls.redraw:
			...
			Graphs.cpu["up"] = Graph(w - bw - 3, (h if CONFIG.cpu_single_graph else hh), THEME.gradient["cpu"],
									 cpu.cpu_upper, round_up_low=True)
			if not CONFIG.cpu_single_graph:
				Graphs.cpu["down"] = Graph(w - bw - 3, hh2, THEME.gradient["cpu"], cpu.cpu_lower,
										   invert=CONFIG.cpu_invert_lower, round_up_low=True)
		...
		if CONFIG.show_battery and cls.battery_activity():
			...
			battery_len: int = len(f'{CONFIG.update_ms}') + (11 if cls.width >= 100 else 0) + len(battery_time) + len(
				f'{cls.battery_percent}')
			battery_pos = cls.width - battery_len - 17
			if (
				battery_pos != cls.old_battery_pos or battery_len != cls.old_battery_len) and cls.old_battery_pos > 0 and not cls.resized:
...
class MemBox(Box):
	...
	def _calc_size(cls):
		...
		if cls.height - (3 if cls.swap_on and not CONFIG.swap_disk else 2) > 2 * item_height:
			cls.mem_size = 3
		...
		if CONFIG.mem_graphs:
			cls.graph_height = round(((cls.height - (2 if cls.swap_on and not CONFIG.swap_disk else 1)) - (
				2 if cls.mem_size == 3 else 1) * item_height) / item_height)
				...
				if cls.swap_on:
					for name in cls.swap_names:
						if CONFIG.swap_disk and CONFIG.show_disks:
							...
							Meters.swap[name] = Graph(cls.mem_meter, cls.graph_height, THEME.gradient[name],
													  mem.swap_vlist[name])
						else:
			...
			if CONFIG.show_disks and mem.disks:
				if CONFIG.show_io_stat or CONFIG.io_mode:
					...
					if CONFIG.io_graph_speeds and not cls.graph_speeds:
						try:
							cls.graph_speeds = {spds.split(":")[0]: int(spds.split(":")[1]) for spds in
												list(i.strip() for i in CONFIG.io_graph_speeds.split(","))}
					...
					for name in mem.disks.keys():
						...
						if CONFIG.io_graph_combined or not CONFIG.io_mode:
							...
							l_vals = [("read", cls.disks_io_h // 2, "free", False),
									  ("write", cls.disks_io_h // 2, "used", True)]

						Graphs.disk_io[name] = {
							_name: Graph(width=cls.disks_width - (6 if not CONFIG.io_mode else 0), height=_height,
										 color=THEME.gradient[_gradient],
										 data=mem.disks_io_dict[name][_name], invert=_invert,
										 max_value=cls.graph_speeds.get(name, 10), no_zero=True)
				...
				if CONFIG.io_mode:
					...
					for name, item in mem.disks.items():
						...
						if CONFIG.show_io_stat and name in Graphs.disk_io:
							...
							if cy > h - 1: break
						out += Mv.to(y + cy, x + cx) + (
							f'Used:{str(item["used_percent"]) + "%":>4} ' if big_disk else "U ")
						...
						if len(mem.disks) * 3 + (len(mem.disks_io_dict) if CONFIG.show_io_stat else 0) <= h + 1:
							...
							cy += 1
							if len(mem.disks) * 4 + (
								len(mem.disks_io_dict) if CONFIG.show_io_stat else 0) <= h + 1: cy += 1
	...
	def _draw_fg(cls):
		...
		if cls.resized or cls.redraw:
			...
			if net.address and w - len(net.nic[:10]) - len(net.address) - 20 > 15:
				out_misc += (
					f'{Mv.to(y - 1, x + 7)}{THEME.net_box(Symbol.title_left)}{Fx.b}{THEME.title(net.address)}{Fx.ub}{THEME.net_box(Symbol.title_right)}{Term.fg}')
		...
		for direction in ["download", "upload"]:
			...
			if stats["redraw"] or cls.resized:
				Graphs.net[direction] = Graph(w - bw - 3, cls.graph_height[direction], THEME.gradient[direction],
											  stats["speed"],
											  max_value=net.sync_top if CONFIG.net_sync else stats["graph_top"],
											  invert=direction != "download", color_max_value=net.net_min.get(
						direction) if CONFIG.net_color_fixed else None, round_up_low=True)
...
class ProcBox(Box):
	...
	def selector(cls, key: str, mouse_pos: Tuple[int, int] = (0, 0)):
		...
		if key in ["up", "k"]:
			...
			if mouse_pos[0] > cls.x + cls.width - 4 and cls.current_y + 1 < mouse_pos[
				1] < cls.current_y + 1 + cls.select_max + 1:
				if mouse_pos[1] == cls.current_y + 2:
					cls.start = 1
				elif mouse_pos[1] == cls.current_y + 1 + cls.select_max:
					cls.start = ProcCollector.num_procs - cls.select_max + 1
				else:
					cls.start = round((mouse_pos[1] - cls.current_y) * (
						(ProcCollector.num_procs - cls.select_max - 2) / (cls.select_max - 2)))
			else:
		...
		if cls.start > ProcCollector.num_procs - cls.select_max + 1 and ProcCollector.num_procs > cls.select_max:
			cls.start = ProcCollector.num_procs - cls.select_max + 1
		elif cls.start > ProcCollector.num_procs:
			cls.start = ProcCollector.num_procs
		if cls.start < 1: cls.start = 1
		if cls.selected > ProcCollector.num_procs and ProcCollector.num_procs < cls.select_max:
	...
	def _draw_fg(cls):
		...
		if cls.resized or cls.redraw:
			...
			if proc.detailed:
				...
				if dw > 28:
					if cls.selected == 0 and not killed and not "K" in Key.mouse: Key.mouse["K"] = [
						[dx + 13 + i, dy - 1] for i in range(4)]
					out_misc += f'{THEME.proc_box(Symbol.title_left)}{Fx.b}{hi}K{title}ill{Fx.ub}{THEME.proc_box(Symbol.title_right)}'
				if dw > 39:
					if cls.selected == 0 and not killed and not "I" in Key.mouse: Key.mouse["I"] = [
						[dx + 19 + i, dy - 1] for i in range(9)]
			...
			if not "f" in Key.mouse or cls.resized: Key.mouse["f"] = [[x + 6 + i, y - 1] for i in range(
				6 if not proc.search_filter else 2 + len(proc.search_filter[-10:]))]
			if proc.search_filter:
				if not "delete" in Key.mouse: Key.mouse["delete"] = [[x + 12 + len(proc.search_filter[-10:]) + i, y - 1]
																	 for i in range(3)]
			...
			if CONFIG.proc_tree:
				label = (f'{THEME.title}{Fx.b}{Mv.to(y, x)}{" Tree:":<{tree_len - 2}}' + (
					f'{"Threads: ":<9}' if tr_show else " " * 4) + (
							 f'{"User:":<9}' if usr_show else "") + f'Mem%{"Cpu%":>11}{Fx.ub}{THEME.main_fg} ' +
				...
				label = (
					...
					((f'{"Threads:":<9}' if arg_len else f'{"Tr:":^5}') if tr_show else "") + (
						f'{"User:":<9}' if usr_show else "") + f'Mem%{"Cpu%":>11}{Fx.ub}{THEME.main_fg} ' +
					(" " if proc.num_procs > cls.select_max else ""))
		...
		if proc.detailed:
			if proc.details["status"] == psutil.STATUS_RUNNING:
				stat_color = Fx.b
			elif proc.details["status"] in [psutil.STATUS_DEAD, psutil.STATUS_STOPPED, psutil.STATUS_ZOMBIE]:
		...
		if cls.start > proc.num_procs - cls.select_max + 1 and proc.num_procs > cls.select_max:
		...
		for n, (pid, items) in enumerate(proc.processes.items(), start=1):
			...
			indent, name, cmd, threads, username, mem, mem_b, cpu = [items.get(v, d) for v, d in
																	 [("indent", ""), ("name", ""), ("cmd", ""),
																	  ("threads", 0), ("username", "?"), ("mem", 0.0),
																	  ("mem_b", 0), ("cpu", 0.0)]]
			...
			out += (f'{Mv.to(y + cy, x)}{g_color}{indent}{pid:>{(1 if CONFIG.proc_tree else 7)}} ' +
					...
					(g_color + (
						f'{username:<9.9}' if len(username) < 10 else f'{username[:8]:<8}+') if usr_show else "") +
					m_color + ((
								   f'{mem:>4.1f}' if mem < 100 else f'{mem:>4.0f} ') if not CONFIG.proc_mem_bytes else f'{floating_humanizer(mem_b, short=True):>4.4}') + end +
					f' {THEME.inactive_fg}{"⡀" * 5}{THEME.main_fg}{g_color}{c_color}' + (
						f' {cpu:>4.1f} ' if cpu < 100 else f'{cpu:>5.0f} ') + end +
					(" " if proc.num_procs > cls.select_max else ""))
...
class Collector:
	...
	* .collect(*collectors: Collector, draw_now: bool = True, interrupt: bool = False): queues up collectors to run'''
	...
	@classmethod
	def collect(cls, *collectors, draw_now: bool = True, interrupt: bool = False, proc_interrupt: bool = False,
				redraw: bool = False, only_draw: bool = False):
...
class CpuCollector(Collector):
	...
	def get_sensors(cls):
		...
		if SYSTEM == "MacOS":
			try:
				if which("coretemp") and subprocess.check_output(["coretemp", "-p"],
																 universal_newlines=True).strip().replace("-",
																										  "").isdigit():
					cls.sensor_method = "coretemp"
				elif which("osx-cpu-temp") and subprocess.check_output("osx-cpu-temp",
																	   universal_newlines=True).rstrip().endswith("°C"):
		...
		if not cls.sensor_method and SYSTEM == "Linux":
			try:
				if which("vcgencmd") and subprocess.check_output(["vcgencmd", "measure_temp"],
																 universal_newlines=True).strip().endswith("'C"):
	...
	def _collect(cls):
		...
		cls.uptime = str(timedelta(seconds=round(time() - psutil.boot_time(), 0)))[:-3].replace(" days,", "d").replace(
			" day,", "d")
		...
		if cls.sensor_method == "psutil":
				...
				for name, entries in psutil.sensors_temperatures().items():
					for num, entry in enumerate(entries, 1):
						if name == s_name and (entry.label == s_label or str(num) == s_label):
							...
							if getattr(entry, "critical", None) != None and entry.critical > 1:
								cls.cpu_temp_crit = round(entry.critical)
							else:
								cls.cpu_temp_crit = 95
							temp = round(entry.current)
						elif entry.label.startswith(("Package", "Tdie")) and cpu_type in ["",
																						  "other"] and s_name == "_-_" and hasattr(
							entry, "current"):
							...
							temp = round(entry.current)
						elif (entry.label.startswith(("Core", "Tccd", "CPU")) or (
							name.lower().startswith("cpu") and not entry.label)) and hasattr(entry, "current"):
							...
							if not cpu_type:
								...
								if not cls.cpu_temp_high or cls.sensor_swap:
									...
									if getattr(entry, "high", None) != None and entry.high > 1:
										...
										cls.cpu_temp_high = 60 if name == "cpu_thermal" else 80
									if getattr(entry, "critical", None) != None and entry.critical > 1:
										cls.cpu_temp_crit = round(entry.critical)
									else:
										cls.cpu_temp_crit = 80 if name == "cpu_thermal" else 95
								temp = round(entry.current)
			...
			try:
				if cls.sensor_method == "coretemp":
					...
					cores = [max(0, int(x)) for x in
							 subprocess.check_output("coretemp", universal_newlines=True).split()]
					...
					temp = max(0, round(
						float(subprocess.check_output("osx-cpu-temp", universal_newlines=True).strip()[:-2])))
					if not cls.cpu_temp_high:
						cls.cpu_temp_high = 85
						cls.cpu_temp_crit = 100
				elif cls.sensor_method == "vcgencmd":
					temp = max(0, round(float(
						subprocess.check_output(["vcgencmd", "measure_temp"], universal_newlines=True).strip()[5:-2])))
					if not cls.cpu_temp_high:
	...
	def _collect(cls):
		...
		for disk in psutil.disk_partitions(all=CONFIG.use_fstab or not CONFIG.only_physical):
			...
			if cls.excludes and disk.fstype in cls.excludes:
				continue
			if filtering and ((not filter_exclude and not disk.mountpoint in filtering) or (
				filter_exclude and disk.mountpoint in filtering)):
				continue
			...
			if io_counters:
				try:
					if SYSTEM != "BSD":
						...
						raise Exception
					disk_read = round(
						(disk_io.read_bytes - cls.disk_hist[disk.device][0]) / (time() - cls.timestamp))  # type: ignore
					disk_write = round((disk_io.write_bytes - cls.disk_hist[disk.device][1]) / (
						time() - cls.timestamp))  # type: ignore
					...
					if len(cls.disks_io_dict[disk.device]["read"]) > MemBox.width:
						del cls.disks_io_dict[disk.device]["read"][0], cls.disks_io_dict[disk.device]["write"][0], \
		...
		if CONFIG.swap_disk and MemBox.swap_on:
			cls.disks["__swap"] = {"name": "swap", "used_percent": cls.swap_percent["used"],
								   "free_percent": cls.swap_percent["free"], "io": ""}
			for name in ["total", "used", "free"]:
...
class NetCollector(Collector):
	...
	def _get_nics(cls):
		...
		for nic in sorted(io_all.keys(), key=lambda nic: (
			getattr(io_all[nic], "bytes_recv", 0) + getattr(io_all[nic], "bytes_sent", 0)), reverse=True):
	...
	def _collect(cls):
		...
		if not cls.nic in cls.stats:
			...
			for direction, value in ["download", io_all.bytes_recv], ["upload", io_all.bytes_sent]:
				cls.stats[cls.nic][direction] = {"total": value, "last": value, "top": 0, "graph_top": 0, "offset": 0,
												 "speed": [], "redraw": True, "graph_raise": 0, "graph_lower": 7}
...
class ProcCollector(Collector):
	...
	collapsed: Dict = {}
	tree_counter: int = 0
	p_values: List[str] = ["pid", "name", "cmdline", "num_threads", "username", "memory_percent", "cpu_percent",
						   "cpu_times", "create_time"]
	sort_expr: Dict = {}
	...
	sort_expr["arguments"] = compile(
		"' '.join(str(p.info['cmdline'])) or ('' if p.info['name'] == 0.0 else p.info['name'])", "str", "eval")
	...
	sort_expr["cpu responsive"] = compile(
		"(p.info['cpu_percent'] if CONFIG.proc_per_core else (p.info['cpu_percent'] / THREADS))", "str", "eval")
	...
	def _collect(cls):
		...
		if CONFIG.proc_tree:
			...
			for p in sorted(psutil.process_iter(cls.p_values + (["memory_info"] if CONFIG.proc_mem_bytes else []), err),
							key=lambda p: eval(sort_cmd), reverse=reverse):
		...
		if cls.detailed and not cls.details.get("killed", False):
			try:
				...
				if not c_pid in cls.processes: attrs.extend(
					["pid", "name", "cmdline", "num_threads", "username", "memory_percent"])
				...
				if c_pid in cls.processes:
					...
					cls.details["cpu_percent"] = round(
						cls.processes[c_pid]["cpu"] * (1 if CONFIG.proc_per_core else THREADS))
				else:
				...
				if isinstance(cls.details["create_time"], float):
					...
					if uptime.days > 0:
						cls.details["uptime"] = f'{uptime.days}d {str(uptime).split(",")[1][:-3].strip()}'
					else:
					...
					if SYSTEM == "BSD":
						if cls.expand > 2:
							if hasattr(cls.details["io_counters"], "read_count"):
								cls.details["io_read"] = f'{cls.details["io_counters"].read_count}'
							else:
								cls.details["io_read"] = "?"
						if cls.expand > 3:
							if hasattr(cls.details["io_counters"], "write_count"):
								cls.details["io_write"] = f'{cls.details["io_counters"].write_count}'
							else:
								cls.details["io_write"] = "?"
					else:
						if cls.expand > 2:
							if hasattr(cls.details["io_counters"], "read_bytes"):
								cls.details["io_read"] = floating_humanizer(cls.details["io_counters"].read_bytes)
							else:
								cls.details["io_read"] = "?"
						if cls.expand > 3:
	...
	def _tree(cls, sort_cmd, reverse: bool, proc_per_cpu: bool, search: List[str]):
		...
		def create_tree(pid: int, tree: defaultdict, indent: str = "", inindent: str = " ", found: bool = False,
						depth: int = 0, collapse_to: Union[None, int] = None):
			...
			if cont:
				if getinfo:
					...
					if getinfo["cmdline"] == err:
						...
						cmd = " ".join(getinfo["cmdline"]) or "[" + getinfo["name"] + "]"
			...
			for child in children:
				create_tree(child, tree, indent + " │ ", indent + " ├─ ", found=found, depth=depth + 1,
	...
	def main(cls):
		...
		while not cls.close:
			...
			if Timer.not_zero() and not cls.resized:
				...
				if CONFIG.background_update: cls.background = f'{THEME.inactive_fg}' + Fx.uncolor(
					f'{Draw.saved_buffer()}') + f'{Term.fg}'
				Timer.stamp()
		...
		while not cls.close:
				...
				for n, opt in enumerate(option_items):
					...
					if opt == selected:
						if isinstance(value, bool) or opt in ["color_theme", "proc_sorting", "log_level", "cpu_sensor",
															  "cpu_graph_upper", "cpu_graph_lower", "temp_scale"]:
							...
							out += ((f'{t_color} {Symbol.left}{v_color}' if type(value) is int else "  ") +
									f'{str(value) + " " + Symbol.enter:^20.20}' + (
										f'{t_color}{Symbol.right} ' if type(value) is int else "  "))
					else:
			...
			if Key.input_wait(Timer.left()):
				...
				if inputting:
					...
					key == "down" and selected_int == len(option_items) - 1 and (page == pages or pages == 0)):
					...
					change_cat = True
													 ...
													 "net_upload", "draw_clock", "tree_depth", "proc_update_mult",
													 "shown_boxes", "net_iface", "io_graph_speeds"]:
					inputting = True
...
class UpdateChecker:
	...
	def _checker(cls):
		try:
			with urllib.request.urlopen("https://github.com/aristocratos/bpytop/raw/master/bpytop.py",
										timeout=5) as source:  # type: ignore
				for line in source:
			...
			if cls.version != VERSION and which("notify-send"):
				try:
					subprocess.run(["notify-send", "-u", "normal", "BpyTop Update!",
									...
									"-i", "update-notifier", "-t", "10000"], stdout=subprocess.DEVNULL,
...
def create_box(x: int = 0, y: int = 0, width: int = 0, height: int = 0, title: str = "", title2: str = "",
			   line_color: Color = None, title_color: Color = None, fill: bool = True, box=None) -> str:
...
def floating_humanizer(value: Union[float, int], bit: bool = False, per_second: bool = False, start: int = 0,
...
def process_keys():
	...
	while Key.has_key():
		...
		if key in ["mouse_scroll_up", "mouse_scroll_down", "mouse_click"]:
			...
			if mouse_pos[0] >= ProcBox.x and ProcBox.current_y + 1 <= mouse_pos[
				1] < ProcBox.current_y + ProcBox.current_h - 1:
				pass
			elif key == "mouse_click":
				key = "mouse_unselect"
			else:
		...
		if "proc" in Box.boxes:
			if key in ["left", "right", "h", "l"]:
				...
				Collector.collect(ProcCollector, proc_interrupt=True, redraw=True)
						 ...
						 "mouse_click", "mouse_unselect", "j", "k"]:
